...
while True:
    ...
    # processsing the result from the model
    ...
    if (len(left_keypoints) != 0 and len(right_keypoints) != 0):
        print('i am in the line of after code')
        ...
        right_writs = occlusionHandling(right_wrist,right_wrist_prev)
        ...
        cv2.putText(frame, getText(left_degree_e), (int(width * 0.31), 15),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (255, 0, 0), 1)

        # Displaying the Right angle
        cv2.putText(frame, getText(right_degree_e), (int(width * 0.31), 35),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (0, 255, 0), 1)
        ...
        cv2.line(frame, (int(right_shoulder[0]), int(right_shoulder[1])), (int(right_elbow[0]), int(right_elbow[1])),
                 (0, 255, 0), 3)
        cv2.line(frame, (int(right_elbow[0]), int(right_elbow[1])), (int(right_wrist[0]), int(right_wrist[1])),
                 (0, 255, 0), 3)
